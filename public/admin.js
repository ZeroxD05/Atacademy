(async function(){
  ...
  document.getElementById('updated').textContent   = new Date(data.generatedAt).toLocaleString('de-DE');

  // Tables
  const tbCountries = document.querySelector('#table-countries tbody');
  data.countries.slice(0, 10).forEach(row => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${row.country || 'Unbekannt'}</td><td>${fmt(row.count)}</td>`;
  ...
  data.referrers.slice(0, 10).forEach(row => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${row.referrer || 'Direkt'}</td><td>${fmt(row.count)}</td>`;
  ...
  new Chart(ctxW, {
    type: 'line',
    data: { labels: w.labels, datasets: [{ label: 'Besuche (Woche)', data: w.values }] },
  ...
  new Chart(ctxM, {
    type: 'bar',
    data: { labels: m.labels, datasets: [{ label: 'Besuche (Monat)', data: m.values }] },
    options: { responsive: true, plugins:{ legend:{ display:false }}, scales:{ y:{ beginAtZero:true } } }
  });
})();